...
class PowerMenuActionController(
    ...
    override fun getAvailabilityStatus(): Int {
        val available = when (preferenceKey) {
            "users" -> UserHandle.MU_ENABLED && UserManager.supportsMultipleUsers()
            "emergency" -> telephony.isDeviceVoiceCapable()
            else -> true
        }
        return if (PowerMenuConstants.getAllActions().contains(preferenceKey) && available) {
            AVAILABLE
        } else {
    ...
    override fun isChecked(): Boolean =
        when (preferenceKey) {
            "emergency" -> actionManager.userConfigContains(preferenceKey) || emergencyAffordance
            else -> actionManager.userConfigContains(preferenceKey)
        }

    override fun setChecked(isChecked: Boolean): Boolean =
        when (preferenceKey) {
            "bugreport" -> {
                actionManager.updateUserConfig(isChecked, preferenceKey)
